...
WHERE nom_modo LIKE '$d%' AND act_modo='$a'
ORDER BY nom_modo ASC";
}
...
//-------------------------------------------------------------------------------------------------------
function GrabarModo($nom)
{
include("../conexion/conexion.php");

	//Verificar si existe este modo 
	$sqlv="SELECT * FROM modo WHERE nom_modo='$nom'";
	$resv=mysql_query($sqlv,$con);
	$rowv=mysql_fetch_array($resv);
	if($rowv['id_modo']!=NULL)
	{
	return $rpta="NO";
	}
	...
	{
	$sql="INSERT INTO modo() VALUES(NULL,'$nom',1)";
	mysql_query($sql,$con);	
	return $rpta="SI";
	}
...
//-------------------------------------------------------------------------------------------------------
function ConsultarModo($cod)
{
include("../conexion/conexion.php");

$sql="SELECT * FROM modo WHERE id_modo='$cod'"; 
...
function ActualizarModo($id_modo,$nom,$act,$e)
{
include("../conexion/conexion.php");
...
function EliminarModo($id_modo)
{
include("../conexion/conexion.php");

	$sqlg="DELETE FROM modo WHERE id_modo='$id_modo'";
	mysql_query($sqlg,$con);
