...
function MostrarPrioridad($p,$d,$a)
{
...
WHERE nom_prioridad LIKE '$d%' AND act_prioridad='$a'
ORDER BY nom_prioridad ASC";
}
...
//-------------------------------------------------------------------------------------------------------
function GrabarPrioridad($nom)
{
include("../conexion/conexion.php");

	//Verificar si existe este prioridad 
	$sqlv="SELECT * FROM prioridad WHERE nom_prioridad='$nom'";
	$resv=mysql_query($sqlv,$con);
	$rowv=mysql_fetch_array($resv);
	if($rowv['id_prioridad']!=NULL)
	{
	return $rpta="NO";
	}
	...
	{
	$sql="INSERT INTO prioridad() VALUES(NULL,'$nom',1)";
	mysql_query($sql,$con);	
	return $rpta="SI";
	}
...
//-------------------------------------------------------------------------------------------------------
function ConsultarPrioridad($cod)
{
include("../conexion/conexion.php");

$sql="SELECT * FROM prioridad WHERE id_prioridad='$cod'"; 
...
function ActualizarPrioridad($id_prioridad,$nom,$act,$e)
{
include("../conexion/conexion.php");
	...
	$resv=mysql_query($sqlv,$con);
	$rowv=mysql_fetch_array($resv);
	$cp=$rowv['nom_prioridad'];
	
	if($cp==NULL || ($cp==$e))
	{
	$sqlg="UPDATE prioridad SET 
	nom_prioridad='$nom',
	act_prioridad='$act'
...
function EliminarPrioridad($id_prioridad)
{
include("../conexion/conexion.php");

	$sqlg="DELETE FROM prioridad WHERE id_prioridad='$id_prioridad'";
	mysql_query($sqlg,$con);
